...
class RSVPCard extends Component {
  ...
  getCSV = () => {
    ...
    let csvContent = 'data:text/csv;charset=utf-8,UID,Email,Phone,First Name,Last Name,College,Major,Attending,California,Bus,Stop,Flight,Shirt,Diet,Allergies,Misc\r\n';

    rsvps.forEach((rsvp) => {
      const application = applications.find(el => el.uid === rsvp.uid);
      csvContent += `${rsvp.uid},${application.email},${application.phoneNumber},${application.name.split(' ')[0]},${application.name.split(' ')[1] || ''},"${application.college}","${application.major}",${attendingEnum[rsvp.attending]},${caliEnum[rsvp.transportation]},${rsvp.buses || ''},"${rsvp.norcal || rsvp.socal || ''}","${rsvp.flight || ''}",${shirtEnum[rsvp.shirt]},"${application.diet}","${application.allergies}","${rsvp.misc || ''}"\r\n`;
    });
